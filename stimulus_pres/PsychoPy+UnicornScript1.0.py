...
# --- Import packages ---
import UnicornPy, pygame, time, shutil, os
...
expName = 'stimulus_presentation'  # from the Builder filename that created this script
expInfo = {
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\MayankRamchandani\\ntx-hack-prelim\\stimulus_pres\\stimulus_presentation.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    # Add Unicorn EEG data acquisition setup here
    FrameLength = 1  # or your desired frame length
    user_duration = 1  # set to your desired experiment duration in seconds

    # Ensure the directory structure exists for Unicorn data
    os.makedirs(os.path.dirname('../data/unicorn_data/'), exist_ok=True)

    # Open Unicorn device
    deviceList = UnicornPy.GetAvailableDevices(True)
    device = UnicornPy.Unicorn(deviceList[0])
    file = open('../data/unicorn_data/unicorn_data.csv', "ab")
    ...
    trialComponents = [Welcome_Message, mouse]
    ...
    presentation = data.TrialHandler(nReps=5.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('face_images.xlsx'),
        seed=None, name='presentation')
    thisExp.addLoop(presentation)  # add the loop to the experiment
    thisPresentation = presentation.trialList[0]  # so we can initialise stimuli with some values
    ...
    if thisPresentation != None:
        for paramName in thisPresentation:
            globals()[paramName] = thisPresentation[paramName]
    
    for thisPresentation in presentation:
        currentLoop = presentation
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('faces.started', globalClock.getTime())
        face.setImage(images)
        # keep track of which components have finished
        facesComponents = [face]
        ...
        numberOfGetDataCalls = int(user_duration * UnicornPy.SamplingRate / FrameLength)
    
        # Main acquisition loop
        for i in range(0, numberOfGetDataCalls):
            
            # Initialize or reset the receiveBuffer
            receiveBuffer = np.zeros(receiveBufferBufferLength, dtype=np.uint8)

            # Receives the configured number of samples from Unicorn and writes to buffer
            device.GetData(FrameLength, receiveBuffer, receiveBufferBufferLength)
            
            # Check if the buffer size is sufficient
            if len(receiveBuffer) < receiveBufferBufferLength:
                print("Warning: Buffer size is smaller than expected. Adjust buffer size or handle incomplete data.")
                break

            # Convert buffer to numpy float array and reshape
            ...
            while continueRoutine and routineTimer.getTime() < 2.0:
                ...
                # if face is starting this frame...
                if face.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    ...
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'face.started')
                    # update status
                    face.status = STARTED
                ...
                # if face is stopping this frame...
                if face.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > face.tStartRefresh + 2-frameTolerance:
                        # keep track of stop time/frame for later
                        face.tStop = t  # not accounting for scr refresh
                        face.frameNStop = frameN  # exact frame index
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 'face.stopped')
                        # update status
                        face.status = FINISHED
        ...
        for thisComponent in facesComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('faces.stopped', globalClock.getTime())
    ...
    # completed 5.0 repeats of 'presentation'
    
    # Stop data acquisition, delete objects, and close file
